...
void Forecast::temp_statistics()
{
  ...
  double total_temp = 0;
  for (auto &day : days) {
    max_temp = day->temp > max_temp ? day->temp : max_temp;
    min_temp = min_temp < day->temp ? min_temp : day->temp;
    total_temp += day->temp;
  }
...
void Forecast::wind_statistics()
{
  ...
  double max_wind = 0;
  for (auto &day : days) {
    if (day->wind_speed > max_wind) {
      windiest_day = day->day;
    }
    max_wind = day->wind_speed > max_wind ? day->wind_speed : max_wind;
  }
  // END: F5
}
