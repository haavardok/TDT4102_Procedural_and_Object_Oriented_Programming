...
// already there and replace it with your own.
    ...
    // Her tek eg oppgåva på ordet og antek at det ikkje er punkta som skal vurderast, men det er minste/største verdi i punkta som skal vurderast.
    if (p1.x <= p2.x) {
        begin.x = p1.x;
        end.x = p2.x;
    }
...
// already there and replace it with your own.
    ...
    /*
        a)
        Her vil eg legge til input frå tastaturet som beskrive i dokumentasjonen for AnimationWindow: "bool AnimationWindow::is_key_down(KeyboardKey key);"
        Vidare vil eg lagre verdien i ein variabel. Skalere opp så camera.translateY() og ...X() har fungerande verdiar.
        Så ville eg ha brukt if-setningar til å fiine ut om knappane var trykt på og calle camera.translateX/Y.

        b)
        Sjå a). Fekk dårleg tid dessverre ://
    */
...
// already there and replace it with your own.
    ...
    int id = coordinate.y*width+coordinate.x;
    // Level::tiles.at(id) = tile;
    // Trur eg her har fått tunnelsyn og får berre error. Kommentert ut for å kunne kompilere.
...
void Level::set_walkable_at(TDT4102::Point coordinate, bool is_walkable)
{
// BEGIN: T7
//
...
// already there and replace it with your own.
    ...
    // Endra variabelnamn på walkable både her og i tasks.h. Ref. insperamelding.
    int id = coordinate.y*width+coordinate.x;
    // walkable.at(id) = is_walkable;
    // Trur eg her har fått tunnelsyn og får berre error. Kommentert ut for å kunne kompilere.
...
// already there and replace it with your own.
    ...
    /*
        Ville her ha brukt begin.x og end.x ov tilsvarande for .y for å finne området som skal fargeleggast.
        Kanskje opprette ein auto anchor_end basert på sluttposisjon på musepeikaren slik at eg kan finne skravert
        område basert på Region(p1,p2).
    */
...
// already there and replace it with your own.
    ...
    /*
        Ville her ha brukt ei for-løkke til å iterere over heile Region og calle set_tile_at() for alle til-ane.
...
// already there and replace it with your own.
    ...
    return TileDescriptor{id, name, value};
...
// already there and replace it with your own.
    ...
    std::ifstream ifs{descriptor_file_path};
    if (!ifs) {
    ...
    while (getline(ifs, line)) {
        TileDescriptor tile_type;
        tile_type = process_line(line);
        imgpool.add_tile(tile_type.id, Tile(tile_type.id, tile_type.walkable, tile_type.filename));
    }
...
// already there and replace it with your own.
    ...
    unsigned int level_width = level.get_width();
    unsigned int level_height = level.get_height();

    os << level_width << "\t" << level_height << std::endl;

    int width_counter{0};
    for (const auto & tile : level.tiles) {
        // Ville kanskje lagt til en if-setning for å sjekke om vi er på siste element i vektoren. Kan da hindre at "\t" blir lagt til på slutten av siste ID i siste ID-line.
        width_counter++;
        if (width_counter==level_width) {
            width_counter = 0;
            os << std::endl;
        } else {
            os << tile << "\t";
    ...
    int counter{0};
    for (const auto & w : level.walkable) {
        // Ville kanskje lagt til en if-setning for å sjekke om vi er på siste element i vektoren. Kan da hindre at "\t" blir lagt til på slutten av siste WA i siste WA-line.
        counter++;
        if (counter==level_width) {
            counter = 0;
            os << std::endl;
        } else {
            os << w << "\t";
