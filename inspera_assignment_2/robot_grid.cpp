...
void RobotGrid::draw_grid_lines()
{
    ...
    window.draw_line({w_size,h_size}, {x_pos,h_size}, TDT4102::Color::black);
    window.draw_line({x_pos,h_size}, {x_pos,y_pos}, TDT4102::Color::black);

    for(int i=1; i<=cols-1; i++) {
      window.draw_line({x_pos+i*cell_width,y_pos}, {x_pos+i*cell_width,h_size}, TDT4102::Color::black);
    }

    for(int i=1; i<=rows-1; i++) {
      window.draw_line({x_pos,y_pos+i*cell_height}, {w_size,y_pos+i*cell_height}, TDT4102::Color::black);
    }
...
void RobotGrid::draw_robots()
{
  ...
  for (const auto &robot : robots) {
    window.draw_circle(get_grid_cell_center_coord(robot.second->pos),min(cell_height/2,cell_width/2),robot.second->color);
